...
<template>
    <div v-if="isModalOpen" class="absolute h-full w-full left-0 top-0 bg-[#0000008e] z-[999]">
        <div
            class="absolute drop-shadow-2xl rounded-xl left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] p-6 bg-white">
            <span>Continuando su questo ristorante il tuo carrello corrente verrà eliminato, sei sicuro di voler
                procedere?</span>
            <div class="flex justify-center gap-4 items-center mt-6 text-white">
                <button @click.prevent="modalHandleClick('no')"
                    class="px-4 py-1 bg-green-500 rounded-md drop-shadow hover:bg-green-600 transition-colors duration-200">
                    No
                </button>
                <button @click.prevent="modalHandleClick('yes')"
    ...
    <section class="bg-image">
        <div class="container mx-auto transition-all duration-200 px-12 relative">
            <div class="flex flex-col justify-center items-center py-4">
                <div :style="{ backgroundImage: `url(${restaurant.photo})` }"
                ...
                <!-- Cart -->
                <div class="absolute top-4 right-0 bg-white rounded-md p-3 shadow-lg border-2 cursor-pointer"
                    @click="toggleCartMenu" v-show="isShowingCart">
                    <font-awesome-icon class="text-3xl" icon="fa-solid fa-cart-shopping" />
                </div>
                <div class="fixed top-0 right-0 bg-black opacity-75 text-white rounded-md lg:shadow-md px-[70px] py-[20px] h-full z-10 flex flex-col justify-between"
                    ...
                    <div>
                        ...
                        <div v-for="dish in cart" :key="dish.id" class="flex justify-between gap-6">
                            ...
                            <div class="flex gap-2">
                                <span @click="removeFromCartHandler(dish)"
                                    class="text-gray-400 cursor-pointer"><font-awesome-icon
                                        icon="fa-regular fa-square-minus" /></span>
                                <span>{{ dish.counter }}</span>
                                <span @click="addToCartHandler(dish)"
                                    class="text-gray-400 cursor-pointer"><font-awesome-icon
                                        icon="fa-regular fa-square-plus" /></span>
                                <span @click="removeDish(dish)" class="cursor-pointer text-red-500"><font-awesome-icon
                    ...
                    <div class="flex flex-col gap-4">
                        ...
                        <span>{{ total }}</span>
                        <button class="border rounded-md" @click="toggleCartMenu">
                            Chiudi
                        </button>
                        <button class="border rounded-md" @click="toggleCartMenu">
                ...
                <div class="border w-[80%] p-4 rounded-2xl flex justify-around gap-10 text-xl mb-10 shadow-md bg-white">
                    ...
                    <button @click.prevent="changeShowingStatus('info')"
                ...
                <div v-show="isShowingMenu" class="relative flex justify-center mb-10">
                    ...
                    <input type="text" id="input-group-1" v-model="input"
                ...
                <div v-show="isShowingMenu" v-for="dish in filteredDishes" :key="dish.id"
                    class="w-full mb-8 bg-white hover:scale-[1.02] transition-all duration-300">
                    <div
                        class="flex justify-between items-center border rounded-md p-6 hover:shadow-lg cursor-pointer transition-all duration-300">
                        <div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <span @click="addToCartHandler(dish)" class="text-gray-400 cursor-pointer"><font-awesome-icon
                                    icon="fa-regular fa-square-plus" /></span>
                            <span @click="removeDish(dish)" class="cursor-pointer text-red-500"><font-awesome-icon
